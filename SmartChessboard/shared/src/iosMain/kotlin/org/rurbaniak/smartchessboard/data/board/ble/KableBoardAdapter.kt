...
class KableBoardAdapter(
    ...
    override suspend fun connect(id: String) {
        ...
        // First-pair tolerance (S-09 Phase 8 fix): the OS pairing dialog adds latency and the link can
        // drop once during bonding, so connect() can throw on the first attempt and succeed on the next
        // once the bond is cached. Retry here, and DON'T mirror Kable's transient Disconnected to the
        // transport state until we're genuinely connected — otherwise the screen flashes "failed" while
        // the pairing dialog is still up (forcing a manual Retry).
