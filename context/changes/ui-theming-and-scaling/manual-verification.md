...
- [x] 1.6 Cycling the History top-bar control (Auto → Light → Dark) switches the theme live on
  Android, iOS, and web.
- [x] 1.7 The chosen mode survives an app restart on each target (persisted via `ui.themeMode`).
- [x] 1.8 Both modes are readable — board, eval bar, diagnostics, surfaces, and text all legible; the
...
- [x] 2.4 On a wide window (web/desktop/tablet) the board auto-fills the available space up to the
  viewport height; dragging the corner handle resizes it; the size survives an app restart
  (persisted via `ui.boardSize`).
- [x] 2.5 On a phone the board is full-width auto-fit with no handle and no regression.
- [x] 2.6 Replay (two-pane and single-column), Play, and PhysicalPlay all render the board correctly at
...
- [x] 3.3 Stepping forward/back through plies, the eval bar holds the prior evaluation and animates to
  the new fraction instead of snapping to the centre while the next eval is fetching.
- [x] 3.4 A loading affordance (the held label pulsing) shows while the new ply's eval is being fetched.
- [x] 3.5 The numeric score label is readable, sits at a fixed bottom-centre spot inside the bar, and
...
- [x] 4.2 In Play, selecting a piece fades the selection tint and legal-target marks in (and out when
...
- [x] 4.3 Navigating between screens animates on Android, iOS, and web (forward slides in from the
...
- [x] 4.4 The promotion and end-game dialogs animate in (fade + slight scale-up); the Replay eval
...
- [x] 5.3 In Play, a tapped move slides the piece from its source to its destination; captures,
  castling, en passant, and promotion animate correctly under both board orientations (flip).
- [x] 5.4 In Replay, stepping forward animates the slide; jumping to an arbitrary ply or loading a
  game renders instantly with no glitch (no piece flying across the board).
- [x] 5.5 In PhysicalPlay (emulator) a resolved physical move slides correctly; selection / lift
...
- [x] 6.3 On a wide window the board resize handle sits on the **board's** bottom-right corner and no
  longer overlaps the eval bar's numeric label.
- [x] 6.4 On a wide window (web maximised / desktop / tablet) the content is centred with side margins
...
- [x] 6.5 In the **mobile** (Android / iOS) Replay analysis view the eval bar is visible to the right
  of the board (previously pushed off-screen).
- [x] 6.6 Stepping through plies with analysis on, the eval-panel tile keeps a constant size between
  the "Analyzing…" and evaluated states — no vertical "jump".
- [x] 6.7 The Replay top-bar move-list toggle switches inline ↔ lichess table; the choice persists
...
- [x] 6.8 On a wide window the History games list is capped (~720 dp) and centred, not stretched
...
- [x] 7.2 At the default board size the two-pane content keeps its centred margins (as before).
- [x] 7.3 Dragging the board larger expands the layout toward the window edges — the board can grow
  past the default margins instead of being trapped inside them.
- [x] 7.4 The gap between the board (eval bar) and the right column is visibly tighter than before.
...
- [x] R.1 Piece-slide no longer overshoots/bounces (spring → tween) and is a touch slower; tune via
  `PIECE_SLIDE_DURATION_MS` in `ChessBoardView.kt`.
- [x] R.2 Signing out in dark mode keeps the Sign-in screen dark (no white flash); the Restoring
  screen is themed too. Confirmed on Android, iOS, and web.
- [x] R.3 The Sign-in screen has a theme-cycle control (Auto → Light → Dark) in the top-end corner;
  the choice persists and is reflected once signed in.
- [x] R.4 The piece-slide has no first-frame flash/teleport on Android and web (previously the piece
  blinked at its destination before sliding from the source); the slide is as smooth as on iOS.
- [x] R.5 Screen transitions (History ↔ game and back) no longer flash white behind the slide — the
