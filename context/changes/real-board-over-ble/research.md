...
- **Bring-up specifics for the on-hardware test rig** (no wire-contract impact): matrix-scan inversion (columns driven LOW, rows read; `square_index = file + 8*rank` unchanged, `main.cpp:60-62,160-171`); **DGT-clock confirmation buttons on ADC1 GPIO34/35** with hysteresis + 4-scan debounce (`pins.h:72-73`, `main.cpp:54-55,138-151`) — *temporary direct-ADC, transistor buffer planned* (`HARDWARE.md:173-177`); requires **common ground + the DGT clock powered on** (`HARDWARE.md:143-181`). ⚠️ **Stale docs:** `firmware/AGENTS.md` and the F-03 `plan.md` still cite GPIO22/23 for the buttons — the authoritative wiring is GPIO34/35 (`pins.h` + `HARDWARE.md`). Strapping-pin watch: ROW6 on GPIO12 (`pins.h:24-26`) — first suspect if the board ever fails to boot/advertise.
