...
## Force-encryption to trigger iOS bonding backfired — plaintext BLE is the MVP posture (provisional)

- **Context**: S-09 (`real-board-over-ble`) needed the mobile app to hold a BLE link to the ESP32 board. The plan's decision B forced link **encryption** on both GATT characteristics (`BLE_GATT_CHR_F_WRITE_ENC` / `_NOTIFY_INDICATE_ENC`, S-09 Phase 2) specifically to make iOS **bond** on first access — the assumption being that iOS needs a bond for a stable link. F-03 had already run stably **plaintext** with nRF Connect (bonding optional, not required).

- **Problem**: On the Phase 8 on-hardware gate the encryption-forced iOS bond **desynced repeatedly**: the phone kept a stale LTK the board no longer matched, tried to encrypt the link with it, failed, and **terminated the connection within ~200 ms** (`central disconnected; reason=531` = HCI `0x13` "remote user terminated"). It recurred every few reconnects and cleared only via a manual **system-Bluetooth "Forget"** on the phone — a peripheral cannot force a central to drop its bond, and the in-app "Forget" only clears the app's remembered id, not the OS LTK. The board's `BLE_GAP_EVENT_REPEAT_PAIRING` handler was correct; the failure was iOS re-encrypting with a stale key instead of re-pairing. Encryption, added as *optional hardening*, broke the *core* connect/reconnect flow on iOS. (A stable plaintext link also then exposed a latent bug: the connection screen re-scanned while the singleton adapter was still connected — a connected board isn't advertising, so it hung; fixed with an "already-connected → don't scan" guard + a scan timeout.)

- **Rule**: For a **dumb-sensor board with a minimal threat model** (a chessboard broadcasting piece positions), do **not** force link encryption just to trigger bonding — the bonding dependency it creates is more fragile on iOS than the security it buys is worth for MVP. Keep the characteristics **plaintext** (`NOTIFY` / `WRITE`, no `_ENC`) — the original §1.8 "trust-on-first-pair" posture, and the F-03-proven one. **Recovery after moving a device off the encrypted build:** re-flash plaintext with `pio run -t erase && pio run -t upload` (the erase clears the board's stored bond) **and** "Forget" the board in the phone's system Bluetooth once (clears the OS LTK). The board is **single-central** — test one device at a time; a busy board stops advertising. This decision is **provisional** — reliable in single-device testing, but must be re-validated across several Android + iOS devices before it's final (roadmap **S-10**). If a hardened MITM-resistant link is ever required, design the reconnection/bond model to survive iOS's stale-LTK behaviour, not just enable `_ENC`.

- **Applies to**: implement, impl-review — any firmware GATT security-flag change and any mobile BLE connect/reconnect work. Surfaced 2026-06-30 (S-09 Phase 8 on-hardware gate).

## Golden screenshots are PNG, not WebP — the `webp-imageio` (luciad) codec fails to read its own writes

- **Context**: The `ui-test-layer` change added Roborazzi JVM golden tests. The harness (`ScreenshotHarness.golden`) originally stored goldens as **lossless WebP** via `io.github.darkxanter:webp-imageio` (`LosslessWebPImageIoFormat`), chosen to keep the committed tree small. Verify compares a freshly-rendered capture against the committed golden, loading the latter through Roborazzi's default `JvmImageIoFormat` loader = `ImageIO.read(file)`.

- **Problem**: The `webp-imageio` bundled reader (`com.luciad.imageio.webp.WebPReader`, native JNI) **non-deterministically returns `null` for a shifting ~40% of valid WebP goldens** — files its *own writer* produced and that reference `libwebp` (PIL/`dwebp`) decodes without error. `ImageIO.read` → null → Roborazzi NPEs (`read(...) must not be null` at `AwtRoboCanvas.load`), failing verify. Proof it is the codec, not the render/files/environment: (1) all goldens decode fine with `libwebp`; (2) which subset is unreadable **changes on every re-record** of the identical screens; (3) it reproduces identically on macOS local and ubuntu CI, in the full suite and running one screenshot class in isolation. The failure had lain latent — earlier CI verifies were green either by landing on a readable subset or as Gradle build-cache hits; a later slice added test *source* files, which busted the `testAndroidHostTest` cache, made the gate genuinely re-run, and surfaced it as a red `main`. Re-recording (locally or on CI) does **not** fix it — it only reshuffles which goldens are unreadable.

- **Rule**: Store Roborazzi goldens as **PNG** — Roborazzi's default AWT writer/loader is the JDK-native `ImageIO` PNG codec: lossless, deterministic, and byte-identical across platforms, with no third-party SPI. Do **not** reintroduce `webp-imageio` (or any non-JDK image codec) for goldens to save tree size; the round-trip reliability of the gate matters far more than a ~2× size delta (the full committed set is well under 1 MB at 0.5 scale, deep under the plan's ~10 MB budget). Keep `resizeScale = 0.5` and the `SimpleImageComparator` AA tolerance; set the golden `filePath` extension to `.png` and leave `RecordOptions.imageIoFormat` at its default (do not pass a custom `ImageIoFormat`). CI stays the canonical recording environment for font-rasterization reasons regardless of format.
...
- **Problem**: `Position` invariants are enforced inconsistently — partly in `Position.init` (board size, en-passant square), partly via a deep `require` (`isInCheck` throws on a missing king), partly not at all. Three latent gaps: (a) `Position` stores the caller's list reference, so a caller holding a `MutableList` can mutate a supposedly-immutable position; (b) a kingless position is constructible and makes `legalMoves`/`validate`/`status` throw `IllegalArgumentException` instead of returning a structured outcome; (c) castling trusts `CastlingRights` without checking a rook actually sits on its home square — with stale rights (common in sloppy FENs) `applyMove` teleports the king and loses the rook. Point (c) is guardrail-adjacent: stale rights + `validate` = a saved illegal move.

- **Rule**: Any code that constructs a `Position` from **outside** data (FEN parse, persistence rehydration, BLE snapshot) must validate at that boundary before handing it to the engine: **both kings present**, **castling rights consistent with the rooks on their home squares**, and the **board list defensively copied** (never retain the caller's reference). The engine itself stays trust-on-input for the internal `start()` + `applyMove` paths (verified: those are the only current constructors), so `applyMove`'s hot path pays no defensive-copy cost. Add the validation in the loader slice that first introduces an external constructor — not retroactively across the engine.

- **Applies to**: plan, implement, impl-review — the FEN-loading slice, persistence rehydration, and any BLE snapshot→`Position` path. Deferred hardening from F-01 impl-review finding F1 (F2, the `GameStatus` name collision, is left as-is: the S-04/05/06 slices shipped, so the pre-calcification rename window closed). Recorded at F-01 close-out (2026-07-04).
