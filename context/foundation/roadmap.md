...
| D      | Firmware       | `F-03`                                         | Firmware software (BLE GATT service) for the ESP32 board — unit-tested + validated against the F-02 emulator's contract; runs fully parallel to Streams A–C from now and joins Stream C at `S-09` for on-hardware integration. **F-03 implemented + impl-reviewed + merged to `main` 2026-06-20 (`306360d`); automated gates green; reed matrix repaired (2026-06-28) and **on-hardware F-03 gates 2.4–3.10 confirmed 2026-06-29** — `S-09` is unblocked and ready to plan.** |
...
Context note (outside the app codebase): firmware status has since advanced well past this baseline. Reed-matrix scanning was hardware-verified (2026-05-28); the BLE GATT game service (F-03) is now implemented, impl-reviewed and merged (2026-06-20, `306360d`, UUIDs assigned). The physical reed matrix has since been **repaired** and the bring-up wiring reworked (diode-direction scan inversion + DGT-clock ADC confirmation buttons, committed 2026-06-28 — `74c7477`/`d948d9e`); only the on-hardware F-03 verification pass remains. This affects only S-09.
...
- **Status:** awaiting review — implemented and impl-reviewed (approved, `cc63cdc`); plan closed out (`001154f`), `change.md` status `impl_reviewed`. Code-complete and perft-verified; **waiting on the user's final review before `/10x-archive`** (no time pressure).
...
- **Status:** implemented — planned, implemented and impl-reviewed; merged to `main` (`bd8640f`). Automated gates green across all KMP targets; `change.md` status `impl_reviewed`. Remaining: the manual-acceptance gate (golden-vector hand-recompute + one cross-target test run) collected in `manual-verification.md`; then `/10x-archive`.
...
- **Status:** awaiting review — implemented and impl-reviewed (phase-6 impl-review recorded `a6f64f9`), `change.md` status `impl_reviewed`. Web sliver (Google sign-in + history) shipped per the 2026-06-10 plan decision. **Waiting on the user's final review before `/10x-archive`** (no time pressure).
...
- **Status:** awaiting review — implemented through Phase 5 on `digital-pass-and-play`: domain PGN writing (SAN writer + PGN serializer, round-tripped against the S-02 parser/corpus, `5c6b327`); the project's first write path — `auth.uid()` column-default migration (`20260612203446`), `createGame`/`updatePgn` repo methods, and the synchronous local write-ahead journal + `GameAutoSaver` realizing the §6.2 ordering invariant (`10d502f`); interactive `ChessBoardView` (tap input, legal-target highlights, orientation/flip — backwards-compatible so Replay renders unchanged) + promotion picker (`9b613ea`); Play & NewGame screens, MVVM ViewModels (MVVM per `lessons.md`, justified in the plan), `PlayKey`/`NewGameKey` routes with browser-history fragments, and History routing by status/mode (`407ab2f`). Three-surface cloud E2E confirmed 2026-06-13 (Android / iOS / web — create → play with castling/capture/promotion → force-quit mid-game → resume with zero lost moves; airplane-mode sync-pending then reconnect flush; cross-surface replay of the cloud copy; stored PGN carries `[Mode "digital"]`, `[Result "*"]`, labels and date, with `user_id` defaulted server-side). Game end & result stay in S-05; the record remains `in_progress` (mate/stalemate only blocks input + shows a banner). `change.md` status `implemented`. **Waiting on the user's final review before `/10x-archive`** (no time pressure).
...
- **Status:** implemented — three-surface E2E verified on Android, iOS, and web (2026-06-17). Pure result mapping + finished-PGN round-trip (`a7a14e1`); atomic `finishGame(status+result+pgn)` repo method + offline-safe finish-aware journal/`GameAutoSaver` (`82c63ec`); `PlayViewModel` auto-close on mate/stalemate + manual end flow (`5759078`); end-game UI — result picker → irreversibility confirm, finished banner, Analyse / Back-to-history — and navigation (`a37a9dd`). E2E follow-up fixes: History list refresh via a repository change signal (`f0e7b04`), hierarchical browser history so web Back follows the stack (`1b1c6a5`), wasm fetch failures handled as `Throwable` so offline no longer crashes (`7fcc3ff`), and a finished game's cloud flush retried until reconnect (`803cdc3`). `change.md` status `impl_reviewed`; awaiting `/10x-archive`.
...
- **Status:** implemented — emulator-driven E2E green on Android/iOS/web (2026-06-19): a magnet-only lift/place stream resolves to exactly one legal move (captures in both orders, interleaved castling, en passant, promotion) into the same canonical PGN as digital play, with promotion blocked until picked and mate/stalemate auto-closing. Sequence interpreter, pure domain (`3dbf1cf`); `createGame(mode)` + `expect/actual supportsPhysicalBoard` + `EmulatedBoard` promoted to `commonMain` (`9dd55cf`); headless MVI core — pure reducer + the §6.2 journal write gated in the `CommitMove` effect (`dc9656b`); physical screen + gated Digital/Physical picker + per-platform DI binding the emulator (`440077d`); emulator-driven E2E + foundation write-backs (p5). The bet is proven without hardware; interactive real-board play over BLE remains S-09. `change.md` status `implemented`; manual code-read/device checks collected in `manual-verification.md`; awaiting `/10x-impl-review` then `/10x-archive`.
...
- **Status:** in progress — waiting on the manual gate. Automated work for all 3 phases implemented and merged to main (`b9bb58e`): headless MVI core — gate, INCONSISTENT, restore-verify (`f5cbf16`); diagnostics UI + screen wiring (`780b6c2`); emulator-driven recover E2E for ILLEGAL + INCONSISTENT (`fa1f868`). Pending the manual device-verification / on-device test pass (see `manual-verification.md`); `change.md` status `implementing`; slice close-out (manual ticks + epilogue + `/10x-impl-review` + `/10x-archive`) still to run.
...
- **CI pipeline (build/test/deploy workflows)** — Partially un-parked. The PR-test gate is owned by `public-repo-and-pr-gate` (S-14); the **release pipeline** (version-tag → gated Cloudflare `wrangler deploy` + downloadable APK GitHub Release) is planned in the `versioned-release-pipeline` change (sequenced after S-14). That change **supersedes** the earlier `github-ci-and-distribution` plan (which folded in Firebase App Distribution + go-public, now split across S-14 and this change).
